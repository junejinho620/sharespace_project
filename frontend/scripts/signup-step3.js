document.addEventListener("DOMContentLoaded", () => {
  ...
  usernameInput.addEventListener("input", async () => {
    ...
    const res = await fetch(
      `http://localhost:5001/api/users/check-username?username=${encodeURIComponent(
        username
      )}`
    );
    const data = await res.json();

    usernameError.textContent = data.exists
  ...
  setupForm.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5001/api/users/${userId}/setup`,
        {
          method: "PUT",
